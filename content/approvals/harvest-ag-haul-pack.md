# Harvest / ag-haul pack — blog + GBP + Facebook + X

**Status:** HTML in repo 2026-09-17 (owner: ship one that's ready). Social still owner-paste only — do not auto-post.  
**Slug:** `harvest-season-clean-truck-check-ag-haul`  
**File:** `blog_drafts/queue/harvest-season-clean-truck-check-ag-haul.md`  
**HTML:** `site/blog/harvest-season-clean-truck-check-ag-haul.html`  
**URL after deploy:** https://norcalcarbmobile.com/blog/harvest-season-clean-truck-check-ag-haul

No personal names in customer-facing copy. Do not auto-post.

---

## Internal links already in the draft (safe set)

- /agricultural-vehicles-clean-truck-check
- /clean-truck-check-blog/ca-ag-vehicles-clean-truck-testing-info
- /clean-truck-check-blog/woodland-agriculture-meets-carb-compliance
- /stockton-clean-truck-check
- /blog/fleets-ovi-obd-porterville-mojave
- /blog/obd-vs-ovi-clean-truck-check-fleets
- /blog/2026-carb-testing-deadlines
- /clean-truck-check-blog/is-carb-clean-truck-check-testing-four-times-this-year
- /qa-and-glossary
- /services /pricing /areas /contact

Do not add `/service-area-san-joaquin-county-mobile-testing` until the $250 leak is gone.

---

## 1) Google Business Profile

Paste after approve. Photo only if the owner has a real farm / shed / harvest-yard shot. Do not invent a diner.

```
Harvest season Clean Truck Check: qualifying ag stays annual — not exempt. The haul truck that takes the crop to the first point of processing is usually still twice a year.

We test at the farm, packing shed, or staging lot in Woodland, Merced, Stockton, and Porterville.

OBD $75 · OVI $199 · we come to the yard.

Book: norcalcarbmobile.com/contact · (916) 890-4427
```

---

## 2) Facebook

**Page:** https://www.facebook.com/carbcleantruckcheck/

```
Harvest does not pause for a station line. It also does not create a blanket exemption.

Qualifying agricultural vehicles stay on an annual Clean Truck Check cycle. The diesel that hauls the crop on public roads is usually still semi-annual — two tests in 2026.

Mobile OBD $75 · OVI $199 at the farm or shed in Woodland, Merced, Stockton, Porterville.

Book: norcalcarbmobile.com/contact · (916) 890-4427
```

After live: add `norcalcarbmobile.com/blog/harvest-season-clean-truck-check-ag-haul`

---

## 3) X / other

```
Harvest CTC: ag can stay annual. Haul trucks cannot. Mobile OBD $75 · OVI $199 at the yard. Woodland / Merced / Stockton. 916-890-4427 norcalcarbmobile.com/contact
```

---

## 4) Retest SMS (calendar description + Night Watch)

```
Hi — Bryan, NorCal CARB Mobile. Ready for next Clean Truck Check? OBD $75 · OVI $199 · we come to your yard. Reply or call 916-890-4427
```
