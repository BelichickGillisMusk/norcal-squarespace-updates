...
2. **Check current site state** — screenshot or scrape each target page first, log what it finds
3. **Execute tasks in order** (Forms → Layout/Font → Blogs)
4. **Verify each change** before moving to the next task — do not assume a save succeeded
5. **Log all changes** to `change_log.md` in this repo with timestamps
6. **Do not publish blog posts** without running them through a spell/grammar check
7. **Do not delete any content** — archive or unpublish only
8. **Stop and log a blocker** if a task cannot be completed — do not skip silently
...
- Pricing: **OBD $75** · **OVI $199**. Both must be accurate and visible on the homepage, services, and pricing pages. Tools and CTAs should show both rates; use the switch offer (*beat your price* or *50% off first test*) for prospects who already have a tester — see `docs/tools-and-cta-strategy.md`.
